...
public class BulkJob {
    ...
    # + return - batch info or error
    public function getBatchInfo(string batchId) returns @tainted error|BatchInfo {
    ...
    public function getAllBatches() returns @tainted error|BatchInfo[] {
        string path = prepareUrl([SERVICES, ASYNC, BULK_API_VERSION, JOB, self.jobId, BATCH]);
        http:Request req = new;
        var response = self.httpClient->get(path, req);
    ...
    public function getBatchRequest(string batchId) returns @tainted error|json|xml|string{
    ...
    # + batchId - batch ID
    # + return - result list
    public function getBatchResult(string batchId) returns @tainted error|json|xml|string|Result[]{
